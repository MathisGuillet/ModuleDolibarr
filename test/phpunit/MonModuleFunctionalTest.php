...
/**
 * \file    test/functional/MonModuleFunctionalTest.php
 * \ingroup monmodule
 ...
 *
 * @package Testmonmodule
 */
class MonModuleFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
...
//        )
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modMonModule')->click();
		} else {
			// Disable the module
			$this->byHref('modMonModule')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/monmodule/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('monmodule/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/monmodule/admin/about.php');
		$this->authenticate();
		return $this->assertContains('monmodule/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('monmodulewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMonModule_MonModuleTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMonModule_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
