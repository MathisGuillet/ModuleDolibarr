...
class modMonModule extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		//$this->automatic_activation = array('FR'=>'MonModuleWasAutomaticallyActivatedBecauseOfYourCountryChoice');
        ...
        */
		...
		$this->cronjobs = array(
			0=>array('label'=>'MyJob label', 'jobtype'=>'method', 'class'=>'/monmodule/class/myobject.class.php', 'objectname'=>'MyObject', 'method'=>'doScheduledJob', 'parameters'=>'', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>true)
		);
		...
		$this->rights[$r][1] = 'Read myobject of MonModule';	// Permission label
		...
		$r++;
		$this->rights[$r][0] = $this->numero + $r;	// Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update myobject of MonModule';	// Permission label
		$this->rights[$r][3] = 1; 					// Permission by default for new user (0/1)
		$this->rights[$r][4] = 'write';				// In php code, permission will be checked by test if ($user->rights->monmodule->level1->level2)
		$this->rights[$r][5] = '';				    // In php code, permission will be checked by test if ($user->rights->monmodule->level1->level2)

		$r++;
		$this->rights[$r][0] = $this->numero + $r;	// Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete myobject of MonModule';	// Permission label
		$this->rights[$r][3] = 1; 					// Permission by default for new user (0/1)
		...
		$this->menu[$r++]=array(	'fk_menu'=>'fk_mainmenu=monmodule',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
								...
								'url'=>'/monmodule/myobject_list.php',
								'langs'=>'monmodule@monmodule',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
								'position'=>1000+$r,
		...
		$this->menu[$r++]=array(	'fk_menu'=>'fk_mainmenu=monmodule,fk_leftmenu=monmodule',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
								...
								'titre'=>'New MyObject',
								'mainmenu'=>'monmodule',
								'leftmenu'=>'monmodule_myobject_new',
								'url'=>'/monmodule/myobject_page.php?action=create',
								'langs'=>'monmodule@monmodule',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
								'position'=>1000+$r,
								'enabled'=>'$conf->monmodule->enabled',  // Define condition to show or hide menu entry. Use '$conf->monmodule->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
								'perms'=>'1',			                // Use 'perms'=>'$user->rights->monmodule->level1->level2' if you want your menu with a permission rules
		...
		$this->export_icon[$r]='myobject@monmodule';
	...
	public function init($options='')
	{
		...
		//$result1=$extrafields->addExtraField('myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'monmodule@monmodule', '$conf->monmodule->enabled');
		//$result2=$extrafields->addExtraField('myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'monmodule@monmodule', '$conf->monmodule->enabled');
		//$result3=$extrafields->addExtraField('myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'monmodule@monmodule', '$conf->monmodule->enabled');
		//$result4=$extrafields->addExtraField('myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1 '', 0, 0, '', '', 'monmodule@monmodule', '$conf->monmodule->enabled');
		//$result5=$extrafields->addExtraField('myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'monmodule@monmodule', '$conf->monmodule->enabled');
